...
<div class="p-8">
    @if($esDividida)
        <div class="mt-4 p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl">
            ...
            <p class="text-white text-sm font-bold">Dividida entre {{ $totalPartes }} personas</p>
            <p class="text-gray-400 text-[10px] uppercase mt-1 font-bold">
                Pago por persona: <span class="text-green-400">${{ number_format(($totalPagar ?? 0) / ($totalPartes > 0 ? $totalPartes : 1), 2) }}</span>
            </p>
        </div>
    @else
    ...
    <div class="px-8 pb-4">
        <div class="flex gap-2 overflow-x-auto pb-2 -mx-2 px-2 scrollbar-hide">
            @foreach($cuentasDividadasInfo as $index => $cuenta)
                ...
                <button 
                    type="button" 
                    class="btn-cuenta px-4 py-2 rounded-xl font-bold text-xs whitespace-nowrap transition-all flex items-center gap-2 border-2 {{ $esPagada ? 'bg-emerald-900/50 border-emerald-500 text-emerald-200 opacity-70 cursor-not-allowed' : 'bg-[#141417] border-blue-500 text-white hover:bg-blue-500/10' }}"
            ...
            @forelse(($cuentasDividadasInfo[0]['productos'] ?? []) as $producto)
                <div class="flex items-center justify-between group">
                    <div class="flex items-center gap-4">
                        <div class="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center text-blue-500 font-black text-xs border border-white/5 group-hover:border-blue-500/30 transition-colors">
                            {{ $producto->cantidad }}x
                        </div>
                        <div>
                            <p class="text-white font-bold text-sm">{{ $producto->nombre }}</p>
                            <p class="text-[10px] text-gray-400">Unit: ${{ number_format($producto->precio_unitario, 2) }}</p>
                            @if($producto->notas)
