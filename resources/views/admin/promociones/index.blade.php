...
<div class="p-4 sm:p-6 lg:p-8 w-full max-w-[1600px] mx-auto space-y-6 sm:space-y-8 relative z-10">
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 sm:gap-6">
        <div class="space-y-2 sm:space-y-3 max-w-2xl w-full">
            <div class="inline-flex items-center gap-2 rounded-full !bg-blue-500/10 px-3 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.35em] !text-blue-600 dark:!text-blue-400 shadow-inner">
                <i class="fas fa-tags"></i> Marketing y Ofertas
            </div>
            <h1 class="text-xl sm:text-3xl md:text-4xl font-black !text-gray-900 dark:!text-white tracking-tighter drop-shadow-sm">Promociones y Descuentos</h1>
            <p class="text-xs sm:text-sm font-medium !text-gray-500 dark:!text-gray-400 tracking-wide">Controla las ofertas activas, paquetes especiales y descuentos para tus clientes.</p>
        </div>
    ...
    <div class="!bg-white dark:!bg-[#121318] rounded-[24px] p-4 flex flex-col lg:flex-row justify-between items-center gap-4 sm:gap-6 border !border-gray-200 dark:!border-white/5 shadow-sm dark:shadow-lg transition-colors">
        ...
        <div class="flex items-center justify-between sm:justify-end w-full lg:w-auto gap-4 sm:gap-6 sm:px-4">
            <div class="text-center sm:text-right flex-1 sm:flex-none">
                <p class="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] !text-gray-500 dark:!text-gray-400">Total Registradas</p>
                <p class="text-xl sm:text-2xl font-black !text-gray-900 dark:!text-white leading-none mt-1">{{ $promociones->count() }}</p>
            </div>
            <div class="w-px h-8 sm:h-10 !bg-gray-200 dark:!bg-white/10"></div>
            <div class="text-center sm:text-left flex-1 sm:flex-none">
                <p class="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] !text-gray-500 dark:!text-gray-400">Activas ahora</p>
    ...
    @if($promociones->isEmpty())
        <div class="!bg-white dark:!bg-[#121318] rounded-[24px] sm:rounded-[32px] p-8 sm:p-12 md:p-20 text-center border !border-gray-200 dark:!border-white/5 shadow-sm dark:shadow-2xl flex flex-col items-center justify-center mt-6 sm:mt-8 relative overflow-hidden transition-colors">
            ...
            <div class="relative mb-6">
                ...
                <div class="w-20 h-20 sm:w-24 sm:h-24 !bg-blue-50 dark:!bg-[#1e2028] rounded-3xl flex items-center justify-center border !border-blue-100 dark:!border-white/5 shadow-inner relative z-10 group hover:!border-blue-500/30 transition-colors">
                    <i class="fas fa-ticket-alt text-4xl sm:text-5xl !text-blue-500/80 group-hover:!text-blue-500 transition-colors"></i>
                </div>
            </div>
            ...
            @foreach($promociones as $promo)
                ...
                <article class="fila-promocion !bg-white dark:!bg-[#121318] border !border-gray-200 dark:!border-white/5 rounded-[24px] p-5 sm:p-6 relative group transition-all duration-300 hover:-translate-y-1 hover:!border-blue-500/30 hover:shadow-xl dark:hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.3)] flex flex-col overflow-hidden">
                    ...
                    <div class="flex justify-between items-start mb-5 sm:mb-6 pt-2">
                        <div class="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-2xl {{ $promo->esta_activa ? 'bg-gradient-to-br from-blue-500 to-indigo-600 !text-white shadow-[0_0_20px_rgba(59,130,246,0.3)]' : '!bg-gray-50 dark:!bg-black/40 !text-gray-400 border !border-gray-200 dark:!border-white/5' }} transition-all duration-300">
                    ...
                    <div class="mb-5 sm:mb-6 flex-1">
                        ...
                        <div class="mt-4 flex items-end gap-1.5 sm:gap-2">
                            <span class="bg-clip-text text-transparent bg-gradient-to-r {{ $promo->esta_activa ? 'from-blue-500 to-indigo-500 dark:from-blue-400 dark:to-indigo-500' : 'from-gray-400 to-gray-500 dark:from-gray-500 dark:to-gray-600' }} font-black text-3xl sm:text-4xl tracking-tighter">
                            ...
                            <span class="text-[9px] sm:text-[10px] font-black uppercase tracking-widest !text-gray-400 dark:!text-gray-500 mb-1.5 pb-0.5">Beneficio</span>
                        </div>
                    </div>

                    {{-- DÍAS DE LA SEMANA --}}
                    <div class="mb-5 sm:mb-6">
                        <p class="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.3em] !text-gray-400 dark:!text-gray-500 mb-2 sm:mb-3">Días aplicables</p>
                        <div class="flex flex-wrap justify-between gap-1.5 sm:gap-1">
                            ...
                            @foreach(['L','M','M','J','V','S','D'] as $i => $d)
                                <div class="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black transition-colors shrink-0 {{ in_array($i+1, $dias) ? '!bg-blue-500 !text-white shadow-[0_0_10px_rgba(59,130,246,0.4)]' : '!bg-gray-50 dark:!bg-black/40 !text-gray-400 dark:!text-gray-500 border !border-gray-200 dark:!border-white/5' }}">
                                    {{ $d }}
                                </div>
                            @endforeach
                        </div>
                    ...
                    {{-- FOOTER DE TARJETA --}}
                    <div class="flex justify-between items-center pt-4 border-t !border-gray-100 dark:!border-white/5 gap-2">
                        <span class="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full text-[8px] sm:text-[9px] font-black uppercase tracking-widest shrink-0 {{ $promo->esta_activa ? '!bg-emerald-50 dark:!bg-emerald-500/10 !text-emerald-600 dark:!text-emerald-500 border !border-emerald-200 dark:!border-emerald-500/20' : '!bg-gray-50 dark:!bg-black/40 !text-gray-500 border !border-gray-200 dark:!border-white/5' }}">
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            @if(auth()->user()->tienePermiso('promociones.editar'))
                                <button onclick="editPromo({{ $promo->id }})" class="flex h-9 w-9 items-center justify-center rounded-lg !bg-gray-50 dark:!bg-black/40 border !border-gray-200 dark:!border-white/5 !text-gray-400 dark:!text-gray-500 transition-all hover:!bg-blue-50 dark:hover:!bg-blue-500/10 hover:!border-blue-200 dark:hover:!border-blue-500/30 hover:!text-blue-500 outline-none active:scale-95">
                                    <i class="fas fa-pen text-xs"></i>
                                </button>
                            @endif

                            @if(auth()->user()->tienePermiso('promociones.eliminar'))
                                <button onclick="openDeleteModal({{ $promo->id }}, '{{ addslashes($promo->nombre) }}')" class="flex h-9 w-9 items-center justify-center rounded-lg !bg-gray-50 dark:!bg-black/40 border !border-gray-200 dark:!border-white/5 !text-gray-400 dark:!text-gray-500 transition-all hover:!bg-rose-50 dark:hover:!bg-rose-500/10 hover:!border-rose-200 dark:hover:!border-rose-500/30 hover:!text-rose-500 outline-none active:scale-95">
