...
<div id="modalEditar-{{ $item->id }}" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/60 backdrop-blur-sm p-3 sm:p-4">
    <div id="modalContainer-{{ $item->id }}" class="relative bg-[#1c1c1e] rounded-2xl w-full max-w-md sm:mx-4 shadow-2xl scale-95 opacity-0 transition-all duration-200 overflow-hidden flex flex-col max-h-[92dvh]">
        ...
        <div class="flex items-center gap-3 sm:gap-4 p-5 sm:p-7 pb-4 sm:pb-5 shrink-0">
            ...
            <div class="min-w-0">
                <h2 class="text-lg sm:text-xl font-black text-white truncate">Editar Insumo</h2>
                <p class="text-[11px] sm:text-xs text-white/30 uppercase tracking-widest mt-0.5 truncate">{{ strtoupper($item->nombre) }}</p>
            </div>
            <button onclick="cerrarModalEspecifico('modalEditar-{{ $item->id }}')"
                class="ml-auto w-8 h-8 flex items-center justify-center rounded-lg text-white/30 hover:text-white hover:bg-white/10 transition-colors outline-none shrink-0">
        ...
        <form action="{{ route('admin.inventario.update', $item->id) }}" method="POST" class="flex flex-col flex-1 min-h-0">
            ...
            <div class="p-5 sm:p-7 space-y-4 sm:space-y-5 overflow-y-auto flex-1 overscroll-contain" style="-webkit-overflow-scrolling: touch;">
                ...
                <div class="space-y-2">
                    ...
                    <select name="unidad_medida" required
                        class="w-full h-12 bg-[#111113] border border-white/8 rounded-xl px-4 text-base sm:text-sm font-semibold text-white outline-none focus:border-[#3B82F6]/60 focus:ring-2 focus:ring-[#3B82F6]/10 transition-all appearance-none cursor-pointer">
                ...
                <div class="grid grid-cols-2 gap-3 sm:gap-4">
                    <div class="space-y-2 min-w-0">
                        <label class="flex items-center gap-2 text-[10px] font-black text-white/40 uppercase tracking-widest">
                            <i class="fas fa-box text-[#3B82F6] text-[10px]"></i> <span class="truncate">Stock Actual</span>
                        </label>
                        <div class="flex items-center h-12 bg-[#111113]/60 border border-white/5 rounded-xl px-4 cursor-not-allowed">
                            <span class="text-sm font-black text-[#3B82F6] truncate">{{ number_format($item->stock_actual, 2) }}</span>
                        </div>
                    </div>
                    <div class="space-y-2 min-w-0">
                        <label class="flex items-center gap-2 text-[10px] font-black text-white/40 uppercase tracking-widest">
            ...
            <div class="flex items-center justify-between px-5 sm:px-7 py-4 sm:pb-7 gap-4 shrink-0" style="padding-bottom: max(1rem, env(safe-area-inset-bottom));">
                <button type="button" onclick="cerrarModalEspecifico('modalEditar-{{ $item->id }}')"
                    class="text-sm font-bold text-white/30 hover:text-white transition-colors uppercase tracking-widest outline-none px-2 py-3 shrink-0">
                    Cancelar
                </button>
                <button type="submit"
