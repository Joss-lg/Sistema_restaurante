...
<div id="modalCrear" class="fixed inset-0 z-[9999] hidden flex items-center justify-center bg-black/50 backdrop-blur-sm p-3 sm:p-4 transition-all duration-300">
    ...
    <div id="createContainer" class="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 w-full max-w-md rounded-[1.5rem] sm:rounded-[2rem] shadow-2xl overflow-hidden transform transition-all duration-500 scale-95 opacity-0 flex flex-col max-h-[95vh]">
        ...
        <div class="p-5 sm:p-8 pb-4 shrink-0 flex justify-between items-center border-b border-zinc-100 dark:border-zinc-900/50">
            <div class="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                    <h3 class="text-lg sm:text-xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight uppercase m-0 leading-tight">Nuevo Artículo</h3>
        ...
        <form action="{{ route('admin.inventario.store') }}" method="POST" class="p-5 sm:p-8 pt-4 sm:pt-6 space-y-4 sm:space-y-5 overflow-y-auto hide-scroll">
            @csrf
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="nombre" required
                    class="w-full h-11 bg-zinc-50 dark:bg-white/5 border border-transparent rounded-xl px-4 sm:px-5 text-xs font-bold text-zinc-900 dark:text-zinc-100 focus:bg-white dark:focus:bg-white/10 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all placeholder:text-zinc-400 dark:placeholder:text-zinc-500"
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    <select name="categoria_id" required
                        class="w-full h-11 bg-zinc-50 dark:bg-white/5 border border-transparent rounded-xl px-4 sm:px-5 text-xs font-bold text-zinc-900 dark:text-zinc-100 focus:bg-white dark:focus:bg-white/10 focus:border-blue-500 outline-none transition-all appearance-none cursor-pointer">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <input type="number" step="0.01" name="precio_compra"
                        class="w-full h-11 bg-zinc-50 dark:bg-white/5 border border-transparent rounded-xl px-4 sm:px-5 text-xs font-black text-zinc-900 dark:text-zinc-100 focus:bg-white dark:focus:bg-white/10 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all placeholder:text-zinc-400 dark:placeholder:text-zinc-500"
                        placeholder="0.00">
                </div>
                <div class="space-y-2">
                    <label class="flex items-center gap-2 text-[8px] sm:text-[9px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-[0.2em] ml-1">
