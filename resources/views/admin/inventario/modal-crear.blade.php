...
<style>
    @media (min-width: 768px) {
        /* Cuando el teclado virtual está activo, alineamos al tope y reducimos altura */
        ...
        body.teclado-virtual-abierto #createContainer {
            /* Forzamos una altura menor para que el teclado virtual no tape los inputs */
    ...
    <div id="createContainer" class="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 w-full max-w-md rounded-[1.5rem] sm:rounded-[2rem] shadow-2xl overflow-hidden transform transition-all duration-500 scale-95 opacity-0 flex flex-col max-h-[95dvh] sm:max-h-[92dvh]">
        ...
        <form action="{{ route('admin.inventario.store') }}" method="POST" class="flex flex-col flex-1 min-h-0">
            ...
            <div class="p-5 sm:p-8 pt-4 sm:pt-6 space-y-4 sm:space-y-5 overflow-y-auto flex-1 overscroll-contain hide-scroll" style="-webkit-overflow-scrolling: touch;">
                ...
                {{-- Selects (no llevan teclado) --}}
                <div class="space-y-2">
                    <label class="flex items-center gap-2 text-[8px] sm:text-[9px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-[0.2em] ml-1">
                        <i class="fas fa-folder opacity-40"></i> Categoría
                    </label>
                    <select name="categoria_id" required class="w-full h-11 bg-zinc-50 dark:bg-white/5 border border-transparent rounded-xl px-4 sm:px-5 text-base sm:text-xs font-bold text-zinc-900 dark:text-zinc-100 focus:border-blue-500 outline-none transition-all appearance-none cursor-pointer">
                ...
                <div class="space-y-2">
                    ...
                    <select name="unidad_medida" required class="w-full h-11 bg-zinc-50 dark:bg-white/5 border border-transparent rounded-xl px-4 sm:px-5 text-base sm:text-xs font-bold text-zinc-900 dark:text-zinc-100 focus:border-blue-500 outline-none transition-all appearance-none cursor-pointer">
