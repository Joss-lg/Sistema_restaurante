<div id="employeeModal" class="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 hidden opacity-0 transition-all duration-500">
    <div id="modalContent" class="modal-inverso relative bg-[var(--m-bg,#ffffff)] border border-[var(--m-border,#e2e8f0)] rounded-[2.5rem] p-10 w-full max-w-[480px] transform scale-95 transition-all duration-500" style="box-shadow: var(--m-shadow);">
        ...
        <form action="{{ route('admin.empleados.store') }}" method="POST" class="p-8 pt-4 space-y-6">
            @csrf
            ...
            <div class="group">
                ...
                <input type="text" id="nombre" name="nombre" required placeholder="Ej. Juan Pérez" class="w-full h-14 bg-[var(--m-input-bg,#f8fafc)] border border-[var(--m-border,#e2e8f0)] rounded-2xl px-6 font-bold text-[var(--m-text,#09090b)] outline-none focus:border-[#3B82F6]">
            </div>

            {{-- SIEMPRE VISIBLE: Rol del Sistema --}}
            ...
            <div class="flex items-center justify-between p-4 bg-[var(--m-input-bg,#f8fafc)] rounded-2xl border border-[var(--m-border,#e2e8f0)]">
                ...
                <input type="checkbox" id="toggleAcceso" name="puede_acceder_pos" value="1" onchange="toggleAccesoFields()" class="toggle-checkbox w-6 h-6 cursor-pointer">
            </div>

            {{-- CONDICIONAL: PIN (Solo visible si marca acceso) --}}
            <div id="accesoFields" class="hidden space-y-6 transition-all duration-500">
                <div class="group">
                    ...
                    <input type="text" id="codigo_empleado" name="codigo_empleado" maxlength="4" placeholder="0000" class="w-full h-14 bg-[var(--m-input-bg,#f8fafc)] border border-[var(--m-border,#e2e8f0)] rounded-2xl px-6 font-black tracking-[0.8em] text-[var(--m-text,#09090b)] outline-none focus:border-[#3B82F6]">
                </div>
            </div>
            ...
            <div class="flex justify-end gap-4 mt-12 pt-8 border-t border-[var(--m-border,#e2e8f0)]">
                ...
                <button type="submit" class="px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest bg-[var(--m-btn-bg,#0f172a)] text-[var(--m-btn-text,#ffffff)]">Guardar Empleado</button>
            </div>
        </form>
    </div>
</div>
